...
final class PastDivesWorker {
    ...
    public func download(recordings: [Recording], deleteAfter: Bool) {
        ...
        backgroudWorker(recordings: recordings) { recording in
            ...
            self.downloadRequest = RecordingsAPI.downloadRecording(recording: recording,
                                                    fileURL: fileURL,
            ...
            { error in
                ...
                if deleteAfter {
                    RecordingsAPI.deleteRecording(sessionId: recording.sessionId) { error in
                        if let error = error {
                            self.show(error: error)
                            return
                        }
    ...
    public func delete(recordings: [Recording]) {
        ...
        backgroudWorker(recordings: recordings) { recording in
            ...
            RecordingsAPI.deleteRecording(sessionId: recording.sessionId)
            { error in
                if let error = error {
                    self.show(error: error)
                    return
